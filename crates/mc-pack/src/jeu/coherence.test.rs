use super::verifier;
use crate::essais::{Atelier, JAR, entree, verrou};
...
fn une_instance_conforme_laisse_passer() {
    ...
    verifier(&verrou(vec![entree("jei", "both", None)]), &instance).expect("tout est là");
}
...
fn un_mod_serveur_ne_compte_pas_cote_client() {
    ...
    let lock = verrou(vec![
        entree("jei", "client", None),
        entree("ledger", "server", None),
    ]);
