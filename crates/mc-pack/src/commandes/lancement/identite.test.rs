use super::choisir;
use crate::commandes::essais::Atelier;
...
async fn un_pseudo_donne_ouvre_une_session_hors_ligne() {
    let session = choisir(Some("Sam".into()))
        .await
...
async fn le_meme_pseudo_donne_toujours_le_meme_joueur() {
    let une = choisir(Some("Sam".into())).await.unwrap();
    let autre = choisir(Some("Sam".into())).await.unwrap();
    assert_eq!(une.uuid, autre.uuid);

    let voisin = choisir(Some("Alex".into())).await.unwrap();
...
async fn sans_session_enregistree_les_deux_issues_sont_donnees() {
    ...
    let erreur = choisir(None).await.expect_err("aucune session");
