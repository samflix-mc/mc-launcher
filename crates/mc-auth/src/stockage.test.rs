...
/// Les trois enveloppes publiques lisent l'emplacement dans l'environnement.
/// Les éprouver ensemble vérifie qu'elles désignent bien le même fichier : une
/// asymétrie entre `enregistrer` et `charger` déconnecterait le joueur à chaque
/// lancement, sans rien dire.
#[test]
fn les_enveloppes_publiques_designent_le_meme_fichier() {
    use super::{charger, effacer, enregistrer};
    ...
    enregistrer(&etat).expect("écriture");
    assert_eq!(chemin(), dir.join("samflix-mc").join("session.json"));
    assert_eq!(charger(), Some(etat));
    effacer().expect("suppression");
