...
pub struct Avancement {
    ...
    ///
    /// « Prêt à jouer » n'est pas une étape qu'on exécute : c'est le résultat
    /// des neuf précédentes. Sans ce drapeau, la fenêtre l'éclairerait comme
    /// une étape en cours et laisserait croire qu'il reste quelque chose à
    /// attendre.
    pub achevee: bool,
    ...
    ///
    /// Faux entre deux lots — pendant la résolution des mods, l'inspection des
    /// jars, l'exécution de l'installateur NeoForge. C'est là que la barre
    /// restait figée à cent pour cent alors que l'étape travaillait encore :
    /// le lot précédent était fini, le suivant pas encore annoncé. Une
    /// progression indéterminée dit la vérité, un « 100 % » ment.
    pub actif: bool,
...
pub struct Suivi {
    phase: Mutex<Phase>,
    achevee: std::sync::atomic::AtomicBool,
...
impl Default for Suivi {
    fn default() -> Self {
        Self {
            ...
            achevee: std::sync::atomic::AtomicBool::new(false),
            note: Mutex::new(None),
...
impl Suivi {
    /// Une phase commence à travailler.
    pub fn phase(&self, phase: Phase) {
        *self.phase.lock().expect("phase") = phase;
        self.achevee.store(false, Ordering::Relaxed);
    }

    /// Le travail s'arrête sur cette phase, qui est un état et non une action.
    ///
    /// Vide le lot au passage : sans cela, la barre du dernier téléchargement
    /// resterait affichée pleine sous « Prêt à jouer », comme si quelque chose
    /// continuait.
    pub fn termine(&self, phase: Phase) {
        *self.phase.lock().expect("phase") = phase;
        self.achevee.store(true, Ordering::Relaxed);
        *self.fichier.lock().expect("fichier") = None;
        self.oublier_le_lot();
    }
    ...
    pub fn photo(&self) -> Avancement {
        ...
        let fichiers = self.fichiers.load(Ordering::Relaxed);
        let fichiers_total = self.fichiers_total.load(Ordering::Relaxed);

        Avancement {
            phase: *self.phase.lock().expect("phase"),
            achevee: self.achevee.load(Ordering::Relaxed),
            ...
            fichiers,
            fichiers_total,
            actif: en_cours(fichiers, fichiers_total),
            debit: debit as u64,
            restant: restant(total, acquis, debit),
        }
    }

    /// Oublie le lot courant : plus rien à montrer d'un téléchargement.
    fn oublier_le_lot(&self) {
        self.recus.store(0, Ordering::Relaxed);
        self.deja_la.store(0, Ordering::Relaxed);
        self.total.store(0, Ordering::Relaxed);
        self.fichiers.store(0, Ordering::Relaxed);
        self.fichiers_total.store(0, Ordering::Relaxed);
    }
}

/// Reste-t-il des fichiers à régler dans le lot annoncé ?
///
/// La réponse décide de ce que la fenêtre dessine : une barre chiffrée, ou une
/// progression indéterminée. Un lot complet mais une étape qui travaille encore
/// — la résolution des mods entre deux passes, l'installateur NeoForge qui
/// tourne dans sa JVM — est exactement le cas où une barre pleine mentirait.
fn en_cours(fichiers: usize, total: usize) -> bool {
    total > 0 && fichiers < total
}
